...
public function topSellingMovies()
{
    try 
    {
        $moviesData = DB::table('tickets')
            ...
            ->where('tickets.Booking_Status', true) 
...
public function UsersSatisfaction()
{
    try {
        $arr = Rating::selectRaw('
                AVG(movie_quality) as movie_quality,
                AVG(hall_cleanliness) as hall_cleanliness,
                AVG(seat_comfort) as seat_comfort,
                AVG(sound_quality) as sound_quality,
                AVG(screen_quality) as screen_quality,
                AVG(food_quality) as food_quality,
                AVG(staff_behavior) as staff_behavior,
                AVG(overall_experience) as overall_experience
            ')->first()->toArray();

    
        $criteria = array_keys($arr);

     
        $values = array_map(fn($k) => round((float)$arr[$k], 2), $criteria);
    ...
    return view('dashboard.statistics.UsersSatisfaction', compact('criteria', 'values'));
}
